 ...
 *
 * $RCSfile: HTMLContentBuilder.cs,v $
 *
 * $Revision: 1.8 $
 *
	...
	public class HTMLContentBuilder
	{
		...
		public string GetFieldAsHtml(Field f)
		{
			...
			if (f.Document is TextDocument)
			{
				...
				{
					AODLException exception		= new AODLException("Exception while trying to build a HTML string from a Field object.");
					exception.InMethod			= AODLException.GetExceptionSourceInfo(new StackFrame(1, true));
					exception.OriginalException	= ex;
		...
		public string GetODFControlAsHtml(ODFControlRef reference)
		{
			if (reference.Document is TextDocument)
			{
				...
				{
					AODLException exception		= new AODLException("Exception while trying to build a HTML string from an ODF Form.");
					exception.InMethod			= AODLException.GetExceptionSourceInfo(new StackFrame(1, true));
					exception.OriginalException	= ex;
