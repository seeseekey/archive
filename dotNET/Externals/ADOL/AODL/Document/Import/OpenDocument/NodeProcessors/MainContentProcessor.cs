 ...
 *
 * $RCSfile: MainContentProcessor.cs,v $
 *
 * $Revision: 1.10 $
 *
...
namespace AODL.Document.Import.OpenDocument.NodeProcessors
{
	...
	internal class MainContentProcessor
	{
		...
		private void AddToCollection(IContent content, IContentCollection coll)
		{
			...
			if (content is ODFControlRef)
			{
				...
				if (this._document is TextDocument)
				{
					...
					if (fc != null)
					{
						fc._contentCollection = coll;
						fc._controlRef = ctrlRef;
					}
		...
		public ODFControlRef CreateControlRef(XmlNode refNode)
		{
			...
			{
				AODLException exception		= new AODLException("Exception while trying to create a Control Reference.");
				exception.InMethod			= AODLException.GetExceptionSourceInfo(new StackFrame(1, true));
				exception.Node				= refNode;
				exception.OriginalException	= ex;
		...
		private EmbedObject CreateEmbedObject(XmlNode ObjNode)
		{
			try
			{
				...
				ObjectRealPath                      = Path.Combine (OpenDocumentImporter.dir,ObjectRealPath);
			...
			try
			{
				...
				foreach(XmlNode nodeChild in row.Node.ChildNodes)
				{
                    ...
                    if ( xn != null )
                    {
                        ...
                        // Internally, the node is no longer repeated, so it seems correct
